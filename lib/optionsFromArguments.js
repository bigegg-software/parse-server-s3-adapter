...
function _parseAsIs(o) {
    return o;
}

function _parseBoolean(o) {
    if (o == true || o == 'true') {
        return true;
    }
...
function BooleanFromEnvironmentOrDefault(options, key, env, defaultValue) {
    return _fromEnvironmentOrDefault_typesafe(options, key, env, defaultValue, _parseBoolean);
}


function _fromEnvironmentOrDefault_typesafe(options, key, env, defaultValue, parser) {
  options[key] = options[key] || parser(process.env[env]) || parser(defaultValue);
...
function fromEnvironmentOrDefault(options, key, env, defaultValue) {
    return _fromEnvironmentOrDefault_typesafe(options, key, env, defaultValue, _parseAsIs);
}
...
const optionsFromArguments = function optionsFromArguments(args) {
  ...
  options = BooleanFromEnvironmentOrDefault(options, 'assumeBucketIsPublicRead', 'S3_ASSUME_BUCKET_IS_PUBLIC_READ', false);
  return options;
}
